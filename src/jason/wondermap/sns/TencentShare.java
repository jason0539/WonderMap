 ...
 * @date 2014-3-3 TODO 实现QQ好友以及Qzone分享一键完成
 *       usage:1.按opensdk说明文档配置好AndroidManifest.xml
 *       2.在@see{TencentShareConstants}中填写相应分享内容
 *       ,当调用无参数分享方法时，刚采用该接口中默认填写内容，当调用有参数分享时，刚采用传入参数 3.调用示例： TencentShare ts =
 *       new TencentShare(Activity,TencentShareEntity); 第二个参数若为空，刚采用第二步中填写默认参数
 *       ts.shareToQQ();//无参数分享到QQ好友 ts.shareToQZone();//分享到QQ空间
 */
...
public class TencentShare implements TencentShareConstants {
	...
	public TencentShare(Activity context, TencentShareEntity entity) {
		...
		shareEntity = entity;
		if (shareEntity == null) {
			shareEntity = new TencentShareEntity(TencentShareConstants.TITLE,
					TencentShareConstants.IMG_URL,
					TencentShareConstants.TARGET_URL,
					TencentShareConstants.SUMMARY,
					TencentShareConstants.COMMENT);
		}
	...
	private void doShareToQQ(TencentShareEntity entity) {
		...
		params.putString(SocialConstants.PARAM_TARGET_URL,
				entity.getTargetUrl());
		...
		params.putString(SocialConstants.PARAM_APPNAME, mContext.getResources()
	...
	private class BaseUiListener implements IUiListener {
		...
		public void onError(UiError e) {
			...
			L.i("QQ", "onError----" + "code:" + e.errorCode + ", msg:"
	...
	public void onQQLoginComplete() {
		if (ready()) {
			BaseApiListener requestListener = new BaseApiListener(
					"get_simple_userinfo", false);
			Bundle params = new Bundle();
			if (tencent != null && tencent.isSessionValid()) {
				params.putString(Constants.PARAM_ACCESS_TOKEN,
						tencent.getAccessToken());
				params.putString(Constants.PARAM_CONSUMER_KEY,
						tencent.getAppId());
				params.putString(Constants.PARAM_OPEN_ID, tencent.getOpenId());
				params.putString("format", "json");
			}
			tencent.requestAsync("user/get_simple_userinfo", params,
					Constants.HTTP_GET, (IRequestListener) requestListener,
					null);
		}
	}
	...
	private class BaseApiListener implements IRequestListener {
				...
				if (ret == 100030) {
					if (mNeedReAuth) {
						Runnable r = new Runnable() {
							...
							public void run() {
								tencent.reAuth(mContext, mScope,
										new BaseUiListener(-1));
							}
						};
		...
		public void onConnectTimeoutException(ConnectTimeoutException arg0) {
			L.i("IRequestListener.onConnectTimeoutException:",
					arg0.getMessage());
